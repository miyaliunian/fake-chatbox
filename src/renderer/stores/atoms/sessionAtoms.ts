...
export const currentSessionIdAtom = atom(
  (get) => {
    ...
    if (idCached && Array.isArray(sessions) && sessions.some((session) => session.id === idCached)) {
      return idCached
    }
    return Array.isArray(sessions) ? sessions[0]?.id : null
  },
...
export const currentSessionAtom = atom((get) => {
  ...
  // Ensure sessions is actually an array before calling find
  let currentMeta = Array.isArray(sessions) ? sessions.find((session) => session?.id === id) : undefined

  if (!currentMeta) {
    const sorted = get(sortedSessionsListAtom)
    currentMeta = Array.isArray(sorted) ? sorted[0] : undefined
  }
  if (!currentMeta) {
    return null
  }
